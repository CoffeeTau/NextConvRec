...
class SequentialRecModel(nn.Module):
    ...
    def add_position_embedding(self, sequence):
        seq_length = sequence.size(1)  # 获取序列长度
        position_ids = torch.arange(seq_length, dtype=torch.long, device=sequence.device)  # 生成位置 ID
        position_ids = position_ids.unsqueeze(0).expand_as(sequence)  # position_ids 的形状变成 (256, 50)，表示每个 batch 里的用户都有相同的 位置索引
        item_embeddings = self.item_embeddings(sequence)  # (B, max_seq_length) -> (B, max_seq_length, hidden)
